 ...
 */
export function useRunSymbol(
  options: UseRunSymbolOptions = {}
): UseRunSymbolReturn {
  const {
  ...
  const [lastRuns, setLastRuns] = useState<
    Record<string, { ts: string; action: string }>
  >({});
  const { toast } = useToast();

  const runSymbol = useCallback(
    async (symbol: string): Promise<RunSymbolResult | null> => {
      ...
      try {
        ...
        setLastRuns((prev) => ({
          ...prev,
          [symbol]: { ts: new Date().toLocaleTimeString(), action },
        }));
        ...
        setLastRuns((prev) => ({
          ...prev,
          [symbol]: { ts: new Date().toLocaleTimeString(), action: "Erreur" },
        }));
        if (enableToast) {
          toast({
